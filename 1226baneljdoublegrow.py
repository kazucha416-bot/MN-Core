...
def main():
    ...
    print(f"2粒子LJ問題倍精度（毎ステップ出力・NR4回）を {filepath} に書き出します...")

    with open(filepath, "w") as f:
        # --- 1. 初期化ブロック ---
        # NR法を4回記述しています
        f.write("""# x-1.3 h=0.001 m=1 ε=1 σ=1 1/m=1 h^2=1e-6
...
""")
        ...
        for k in range(loop_count):
            # L1BMのアドレス計算 (毎ステップずらす)
            lb_addr = k * 4

            # 物理計算 + エネルギー計算 (条件分岐なしで結合)
            # NR法も4回になっています
            f.write(f"""nop/2
...
""")
        ...
        # --- 3. 終了処理 ---
        # 全データ分 (2000 * 4) を取得
        l1bm_size = loop_count * 4
        f.write(f"""d getd $lm0n0c0b0m0p0 10
d getd $ln0n0c0b0m0p0 4
d getd $lr0n0c0b0m0p0 12
d getd $ls0n0c0b0m0p0 8
d getd $lb0n0c0b0 {l1bm_size}
""")

    print(f"完了しました。L1BM出力サイズ: {l1bm_size}")
