...
def hex_to_double(hex_str):
    """16進数文字列を倍精度浮動小数点数に変換"""
    s = hex_str.strip().replace('0x', '')
    try:
        # big-endian doubleとして解釈
...
def main():
    ...
    output_filename = 'MNCore2_vs_CPU_LJ_Energy_Double.pdf'
    ...
    if os.path.exists(cpu_file):
        try:
            ...
            # --- 間引き処理 (Downsampling) ---
            # CPUは2001点(step=0~2000)、MN-Coreは201点
            step = 10
    ...
    # CPU (Reference) - 黒実線
    ax.plot(cpu_time, cpu_energies, label='CPU (Double Precision)', 
             color='black', linewidth=2.0, alpha=0.8)

    # MN-Core - 赤破線
    ax.plot(cpu_time, mn_energies, label='MN-Core 2 (Double Precision)', 
