# 1次元LJばね問題におけるMN-Core2とCPU（倍精度版）のエネルギー比較グラフを作成するスクリプト
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def hex_to_double(hex_str):
    """
    16進数文字列を倍精度浮動小数点数(Double)に変換
    エラー対策：改行やスペースを除去し、16文字（8バイト）であることを確認
    """
    try:
        s = hex_str.strip().replace('0x', '')
        # 文字列が空、または長さがおかしい(Doubleは16文字)場合はNoneを返す
        if len(s) != 16:
            return None
        # big-endian double (>d)
...
def main():
    ...
    if os.path.exists(cpu_file):
        try:
            # 警告対策: delim_whitespace=True -> sep=r'\s+'
    ...
    if len(mn_energies) == 0:
        print("【エラー】MN-Coreデータが読み込めませんでした（0件）。Hex形式を確認してください。")
        return
    ...
    print("-" * 60)
    print("Error Analysis (MN-Core 2 vs CPU [Double Precision])")
    print("-" * 60)
